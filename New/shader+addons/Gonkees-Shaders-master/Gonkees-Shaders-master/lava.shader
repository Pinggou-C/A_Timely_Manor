...
void fragment() {
	...
	float noise1 = fbm(coord + vec2(TIME * -0.45, TIME * 0.3));
	float noise2 = fbm(coord + vec2(TIME * -0.065, TIME * 0.05));
	
	float combined = noise1 * noise2 + (noise2 * 0.1);
	
	COLOR = vec4(vec3(combined), 1.0);
	
	if (combined > 0.2) {
		COLOR = mix(red, yellow, (combined - 1.0) * 2.0 + 1.0);
	} else {
		COLOR = mix(grey, black, combined * 4.0);
	}
}
